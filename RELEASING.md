...
4. CI builds a static Linux amd64 binary, Debian package and a self-contained
   ...
   It publishes SHA-256 files, signs every artifact with Sigstore and attaches
   provenance.
5. Verify the GitHub release before using `updater update`.

Kernel Register must contain `repositories.updater.url`. The updater
self-update resolves only `updater-v*` tags and the signed
...
CI downloads the bundle from this repository and verifies its checksum and
GitHub OIDC Sigstore identity before including it in a Compose bundle.
